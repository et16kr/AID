...
- `.codex-jobs/llm-reference-consolidation/` workflow는 보존하되, 영문 source 문서가 한국어 원문 기준으로 충분히 완성되기 전에는 실행하지 않는다.
- 취합 workflow 실행 전에는 `arch/`와 `FAQE/` 영문 source 문서에 한국어 원문 기준 누락, 미번역, 오래된 절차, 잘못된 버전 정보, 첨부 링크 누락이 없는지 먼저 보강한다.
- source 문서 보강 전에 `llm-reference/`를 생성하면 이후 변경 사항을 취합 문서에 다시 반영해야 하므로 재작업과 출처 추적성 저하가 생긴다.
...
단계별 gate는 리뷰 보고서나 build report에 명시적으로 기록한다. 다음 단계는 이전 단계 gate가 통과한 뒤에만 시작한다.
...
1. 한국어 원문 기준으로 영문 source 문서의 완성도를 먼저 보강한다.
2. source 문서 보강 후 `manifest.json`, 문서 수, 첨부 링크, 잔여 한글/깨진 링크 검증을 통과시킨다.
3. 그 다음 `.codex-jobs/llm-reference-consolidation/` workflow를 실행해 `llm-reference/` 같은 별도 출력 디렉터리를 만들고 위 출력 문서 이름으로 빈 골격을 만든다.
4. 각 문서의 `Source paths` 절을 먼저 채운 뒤 내용을 취합한다.
5. 취합 중 발견한 한국어-영어 의미 차이는 통합 문서에서 직접 해결하지 말고 원본 영어 문서 보정 작업으로 기록한다.
6. 문서별로 중복 제거, 용어 보존, 링크 보존, 버전 조건 보존을 self-review한다.
7. 전체 패키지에 대해 `rg` 기반 용어 보존 점검, 빈 Markdown 링크 점검, 첨부 URL 보존 점검, `git diff --check`를 수행한다.
8. 최종 산출물마다 한국어 권위 기준, 영어 source paths, 남은 리스크를 남긴다.
