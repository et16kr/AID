---
title: "09-28. Not found data"
page_id: "10060525"
space_key: "faq"
space_name: "FAQ(한국어)"
source_url: "https://docs.altibase.com/display/faq/09-28.+Not+found+data"
updated_at: "2025-08-07T16:21:42.000+0900"
version: 9
...
Source: https://docs.altibase.com/display/faq/09-28.+Not+found+data
Updated: 2025-08-07T16:21:42.000+0900

- [개요](#id-09-28.Notfounddata-개요) - [버전](#id-09-28.Notfounddata-버전) - [현상](#id-09-28.Notfounddata-현상) - [원인](#id-09-28.Notfounddata-원인) - [커서(CURSOR) OPEN 상태에서 COMMIT/ROLLBACK 수행한 경우](#id-09-28.Notfounddata-커서(CURSOR)OPEN상태에서COMMIT/ROLLBACK수행한경우) - [조치](#id-09-28.Notfounddata-조치) - [커서(CURSOR) OPEN 상태에서 COMMIT/ROLLBACK 수행한 경우](#id-09-28.Notfounddata-커서(CURSOR)OPEN상태에서COMMIT/ROLLBACK수행한경우.1) - [1. fetch 세션과 변경 DML 세션 분리](#id-09-28.Notfounddata-1.fetch세션과변경DML세션분리) - [2. 통신 버퍼에 담길 만큼만 커서 선언 후 반복적 커서 오픈](#id-09-28.Notfounddata-2.통신버퍼에담길만큼만커서선언후반복적커서오픈) - [참고](#id-09-28.Notfounddata-참고) - [버전 별 차이](#id-09-28.Notfounddata-버전별차이)
...
위와 같이 커서를 사용하였으나 '3. 커서 FETCH' 단계에서 어느 정도 FETCH 진행하다가 어느 순간 FETCH 할 레코드가 남아있음에도 Not found data 에러가 발생한다.
...
while(1)
{
    ...
    {
        /* 어느 정도 FETCH 진행하다가 어느 순간 fetch 할 레코드가 남아있음에도 Not found data 에러가 발생한다. */
...
SQL_NO_DATA Error : [100][02000] Not found data                       /* fetch 수행 중 에러 발생 */
...
Altibase 5 이상 버전의 통신 버퍼 크기는 64K이다. 통신 버퍼에 담기는 레코드 수는 레코드 크기에 따라 다르기 때문에 LIMIT절 마지막 값은 운영 환경에 따라 달라진다.
...
| Altibase 6.3.1 이상 | ERR-410D2 | Fetch out of sequence. | [http://aid.altibase.com/x/9oKZ](http://aid.altibase.com/x/9oKZ) |
