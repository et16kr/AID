...
1.BUFFER_AREA_SIZE(unit: bytes) = (The bigger, the better.) 2.SORT_AREA_SIZE(unit: bytes) = (Number of physical cores in the system * 20MB) 3.DISK_INDEX_BUILD_MERGE_PAGE_COUNT(unit: page수) = (1% of BUFFER_AREA_SIZE is recommended, but you should consider DISK_INDEX_BUILD_MERGE_PAGE_COUNT (see Consideration #3 below).) 4.INDEX_BUILD_THREAD_COUNT(number of core) = (Number of physical cores in the hardware)
...
- If the property value is large but the index size is small, performance may actually degrade.
  - Index build performance may degrade if the following conditions are met.
   DISK_INDEX_BUILD_MERGE_PAGE_COUNT  > ( Index key length * number of RECORD ) / SORT_AREA_SIZE

**버전 7.3.0 ~**

---

1.BUFFER_AREA_SIZE(unit: bytes) = (The bigger, the better.) 2.DISK_INDEX_BUILD_SORT_AREA_SIZE(unit: bytes) = (Number of physical cores in the system* 20MB) 3.DISK_INDEX_BUILD_MERGE_PAGE_COUNT(unit: number of page) = (1% of BUFFER_AREA_SIZE) 4.INDEX_BUILD_THREAD_COUNT(core수) = (Number of physical cores in the hardware)  ****Considerations****
