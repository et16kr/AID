...
Create the `/etc/init.d/alti_start` file. This script starts the DBMS by using the Altibase startup command.
...
ADMIN="${ALTIBASE_HOME}/bin/isql -s 127.0.0.1 -u sys -p manager -sysdba"
${ADMIN} <<EOF
startup
quit
EOF
...
This script stops the DBMS by using the Altibase command.
...
Create a hard link of the startup and shutdown script in the `/etc/rc3.d` directory.
