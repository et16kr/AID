---
title: "24. Altibase 운영을 위한 AIX 설정 가이드"
page_id: "13436846"
space_key: "DOCK"
space_name: "Technical Documents(한국어)"
source_url: "https://docs.altibase.com/pages/viewpage.action?pageId=13436846"
updated_at: "2023-11-02T13:24:41.000+0900"
version: 36
ancestors: ["Home"]
labels: []
---

# 24. Altibase 운영을 위한 AIX 설정 가이드
Source: https://docs.altibase.com/pages/viewpage.action?pageId=13436846
Updated: 2023-11-02T13:24:41.000+0900

- [개요](#id-24.Altibase운영을위한AIX설정가이드-개요) - [커널 파라미터](#id-24.Altibase운영을위한AIX설정가이드-커널파라미터) - [Posix AIO](#id-24.Altibase운영을위한AIX설정가이드-PosixAIO) - [파일캐시](#id-24.Altibase운영을위한AIX설정가이드-파일캐시) - [리소스 제한](#id-24.Altibase운영을위한AIX설정가이드-리소스제한) - [변경 방법](#id-24.Altibase운영을위한AIX설정가이드-변경방법) - [사용자 환경 설정](#id-24.Altibase운영을위한AIX설정가이드-사용자환경설정) - [리소스 제한](#id-24.Altibase운영을위한AIX설정가이드-리소스제한.2) - [환경변수](#id-24.Altibase운영을위한AIX설정가이드-환경변수) - [요약](#id-24.Altibase운영을위한AIX설정가이드-요약) - [설정 예제](#id-24.Altibase운영을위한AIX설정가이드-설정예제) - [별첨](#id-24.Altibase운영을위한AIX설정가이드-별첨) - [AIX 메모리 관련 패치](#id-24.Altibase운영을위한AIX설정가이드-AIX메모리관련패치) - [IPC 채널 개수 제한](#id-24.Altibase운영을위한AIX설정가이드-IPC채널개수제한)
...
| --- | --- | --- |
| minperm | 10% | - |
| lru_file_repage | 0 | AIX 5.2 ML5, AIX 5.3 ML2 이상부터 지원 |
| strict_maxclient | 0 | AIX 5.2 ML4 이상부터 지원 |
...
| 커널파라미터 | 설명 | 권장 값 |
| --- | --- | --- |
| Maximum number of PROCESSES allowed per user | 사용자당 생성 가능한 프로세스의 개수 | 동시에 구동 가능한 프로세스 개수 이상 |
...
| 환경변수 | 설명 |
| --- | --- |
| AIXTHREAD_MNRATIO | n개의 유저쓰레드 처리를 위한 k개의 커널쓰레드의 개수 |
| AIXTHREAD_SCOPE | 쓰레드모델을 1:1로 설정 |
| AIXTHREAD_MUTEX_DEBUG | pthread 라이브러리가 사용중인 mutex, condition 변수,<br>디버거에서 사용하는 read/write lock에 대한 오버헤드를<br>제거하기 위해 설정 |
| AIXTHREAD_RWLOCK_DEBUG |  |
| AIXTHREAD_COND_DEBUG |  |
| SPINLOOPTIME | CPU자원 획득상태에서 Lock을 획득하기 위해 시도하는 횟수 |
| YIELDLOOPTIME | Lock을 가진 상태에서 CPU자원을 양보하는 횟수 |
| MALLOCMULTIHEAP | malloc을 많이 사용하는 멀티쓰레드 환경에 설정 |
| AIXTHREAD_MUTEX_FAST | 운영체제의 내부적인 mutex_locking 동작방식을 변경하는 옵션으로<br>mutex_contention이 심한 경우 ON으로 설정하여 성능개선 효과를<br>나타낼 수 있다. AIX5.2이상에서 제공된다. |
...
| 분류 | 커널파라미터 | 권장 값 | 비고 |
| --- | --- | --- | --- |
| Posix AIO | Configure Defined Asynchronous I/O | Available | AIX6.1 미만 시 필수 |
| 파일캐시 | lru_file_repage | 0 | AIX6.1 미만 시 고려<br>(lru_file_repage 필수) |
| strict_maxclient | 0 |  |  |
| minperm | 10 |  |  |
| 리소스 제한 | Maximum number of PROCESSES allowed per user | 동시에 구동 가능한 프로세스 개수 이상 | max user process 에 대응 |
...
| 항목 | 설명 | 권장 값 |
| --- | --- | --- |
| data seg size(data) | 프로세스 데이터영역의 최대크기 | unlimited |
| file size (fsize) | 생성되는 파일의 최대크기 | unlimited |
| open files (nofiles) | 하나의 프로세스가 동시에 접근 가능한 파일의 최대개수 | unlimited |
| max memory size (rss) | 사용 가능한 메모리의 최대크기 | unlimited |
| max user process | 사용자당 생성 가능한 프로세스의 개수 | Maximum number of PROCESSES allowed per user |
