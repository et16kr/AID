...
Moreover, the performance of the application is bound to decrease as much as the cost of each connect/disconnect. Therefore, it is recommended for the user to change the structure of an application so that it is processed while connected to the DB. If this is not possible, the malicious query must be retrieved with the profiling function as follows, even at the risk of degrading the DB performance.

```
ALTER SYSTEM SET TIMED_STATISTICS=1;
ALTER SYSTEM SET QUERY_PROF_FLAG=1;  -- Execute the function
ALTER SYSTEM SET QUERY_PROF_FLAG=0;  /* Terminate function */
```
