...
Previously, we discussed how to add the basic APRE library in the "How to make a basic Makefile" chapter. This chapter describes the library and compile options that must be added to the Makefile in an environment that uses the SUN (Solaris) cc compiler.
...
| --- | --- | --- |
| Thread Library | **-lpthread or -lthread** | Thread library for POSIX thread functions |
...
| -xarch=v8plus | When compiling a 32bit program on a SUN SPARC CPU platform |  |
...
An example of an APRE Makefile for SUN X86 cc that specifies the 32bit compile option is as follows. Specify `-xarch=sse2a` as a compile option, and specify the path where the 32bit APRE library is installed in the path that references the header file and library. In addition, the APRE precompiler also specifies the path to execute 32bit APRE.
