...
To use unixODBC Manager, the software binary must be built for the target platform. This chapter describes the necessary parts of each step. It assumes the native compiler. For gcc/g++, there are no special options to note, so gcc/g++ is not described separately. However, the compile options must still be set carefully when creating 32-bit or 64-bit binaries.
...
The bit types of unixODBC and the ODBC driver must match. If unixODBC is compiled as 64-bit, the Altibase ODBC driver must also be 64-bit. If unixODBC is compiled as 32-bit, use the dedicated 32-bit Altibase ODBC driver.

### Setting the size of SQLLEN and SQLULEN type

SQLLEN and SQLULEN types are used when declaring parameter variable types for unixODBC functions. The size of `SIZEOF_LONG_INT` determines whether SQLLEN and SQLULEN are 32-bit or 64-bit. SQLLEN and SQLULEN are declared as 32-bit on a 32-bit OS and 64-bit on a 64-bit OS.
...
Using `export CFLAGS=-DBUILD_LEGACY_64_BIT_MODE=1` is strongly recommended so that SQLLEN uses 4 bytes (32 bits). When SQLLEN/SQLULEN is used as 64-bit (8 bytes), value-transfer errors can occur between unixODBC and Altibase.
...
Also, `sqltypes.h` provided by Altibase uses 32-bit SQLLEN when no special compile option is given. Therefore, when an application uses Altibase `sqltypes.h`, SQLLEN is passed to unixODBC Manager as 32-bit.

However, if unixODBC Manager is built without a separate option, SQLLEN is used as 64-bit. Because of the SQLLEN length mismatch between the application and unixODBC Manager, ODBC functions such as `SQLfetch()` and `SQLMoreResult()` can intermittently return incorrect values.

To prevent this, it is recommended to use only 32bit unixodbc SQLLEN.

### Selecting unixODBC compilation bit type

unixODBC can be selectively compiled as 32-bit or 64-bit by setting compilation environment variables. It is compiled as 64-bit by default on a 64-bit OS. To compile unixODBC as a 32-bit program on a 64-bit OS, set the variables as follows.
...
The options for CFLAGS and LDFLAGS must also be specified differently according to the 32-bit or 64-bit compile option. Otherwise, an ELF class error may occur.
...
The directory where unixODBC is installed is the path set by the `prefix` argument. For example, if `prefix=/home/unixODBC`, unixODBC is finally installed in `/home/unixODBC` after the install operation.
