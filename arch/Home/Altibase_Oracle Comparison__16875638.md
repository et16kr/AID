...
| DeadLock Detection | Auto deadlock detection and recovery (Auto Deadlock Detect & Recovery) | Auto deadlock detection and recovery (Auto Deadlock Detect & Recovery) |  |
...
| Number of tablespaces | 65,533 | 65,536 | Maximum number per database |
...
| Number of constraints | Unlimited | System-wide<br>2,097,151 | Not divided by column |
...
| Recovery Manager | RMAN | iSQL, aexport, iLoader | Can be integrated with storage vendor tools (API provided) |
...
| Management command | Tablespace change | - | Supported | Refer to details of the partition table function |
...
| --- | --- | --- | --- |
| Tablespace change | ALTER TABLE PARTITION | ALTER TABLE PARTITION | ALTER TABLE PARTITION |
...
| Incomplete Recovery | Supported | Supported |  |
| Complete Recovery | Supported | Supported |  |
...
| Hierarchical query<br>CONNECT BY ~ WITH | Supported | Supported | CONNECT_BY_ISCYCLE is not supported |
...
| CREATE TABLE AS SELECT ~ | Supported | Supported |  |
...
| Parallel Select | Supported | Not supported |  |
...
|  | LONG RAW | Variable-length raw binary type. Up to 2G |  | Use BLOB |
|  |  | BYTE | 1 to 32000, fixed-length binary data type |  |
|  |  | NIBBLE | 1 to 254, variable-length binary data type |  |
|  |  | BIT | 1 to 60576, consisting only of 0 and 1<br>Fixed-length binary data type |  |
...
|  | BITAND | BITAND | Return an integer by performing an AND operation on the bits of argument 1 and argument 2 |
|  | BITNOT | BITNOT | Return the result of a NOT operation on the bits of bit_a |
|  | BITOR | BITOR | Return the result of an OR operation on the bits of bit_a and bit_b |
...
|  | Not supported | ISNUMERIC | Determine whether the entered expression is valid as a numeric data type |
|  | LN | LN | Return the natural logarithm of the input value |
|  | LOG | LOG | Return the logarithm of n with base m in LOG(m,n) |
|  | MOD | MOD | Return the remainder of dividing n2 by n1 |
|  | NANVL | Not supported | If input value n2 is NaN (not a number), return replacement value n1. If n2 is not NaN, return n2 |
|  | Not supported | NUMAND | Return the result of the bitwise AND operation of BIGINT type bigint_a and bigint_a as a BIGINT type value |
|  | Not supported | NUMOR | Return the result of the bitwise OR operation of BIGINT type bigint_a and bigint_a as a BIGINT type value |
|  | Not supported | NUMSHIFT | Return the result of shifting BIGINT type bigint by n bits as a BIGINT type value |
|  | Not supported | NUMXOR | Return the result of the bitwise XOR operation of BIGINT type bigint_a and bigint_a as a BIGINT type value |
|  | POWER | POWER | Return the value of n2 to the power of n1 |
|  | Not supported | RAND | Generate a random number between 0 inclusive and 1 exclusive and return it as a double type value |
|  | dbms.random() | RANDOM | Return a pseudo-random integer value |
|  | REMAINDER | MOD | Return the remainder of n2 divided by n1 |
|  | ROUND (number) | ROUND (number) | Round n at the decimal place based on integer and return the result |
...
|  | TRUNC (number) | TRUNC (number) | Truncate argument n1 to the decimal place specified by parameter n2 |
|  | WIDTH_BUCKET | Not supported | Create a histogram with equal-width buckets |
| Character functions that return character values | Not supported | CHOSUNG | Extract and return only the first letter of each letter from the entered Hangul string |
|  | CHR | CHR | Return the ASCII code corresponding to decimal number n |
|  | CONCAT | CONCAT | Concatenate char1 and char2 and return the result |
|  | Not supported | DIGEST | Return the hash digest of expr as a VARCHAR type using a quasi-encrypted hash algorithm |
|  | Not supported | DIGITS | Return the input integer as a string |
|  | INITCAP | INITCAP | Convert the first letter of each word in the input string to uppercase and the rest to lowercase, then return the result |
|  | LOWER | LOWER | Convert the input string to lowercase |
|  | LPAD | LPAD | Fill expr1 from specified length n and fill the remaining left-side space with expr1 |
|  | LTRIM | LTRIM | Remove all characters specified by set from the left side of character string char |
|  | NCHR | NCHR | Return a Unicode character |
|  | NLS_INITCAP | Not supported | Return char after converting the first letter of each word to uppercase and the remaining letters to lowercase |
|  | NLS_LOWER | Not supported | Convert all characters to lowercase and return the result |
|  | NLS_UPPER | Not supported | Return the input string converted to uppercase |
|  | NLSSORT | Not supported | Sort the input string and return a string |
|  | Not supported | RANDOM_STRING | Create a random string with length characters in the format specified by option |
...
|  | REGEXP_SUBSTR | REGEXP_SUBSTR | Return the substring that satisfies the specified regular expression |
|  | REPLACE | REPLACE2 | Replace every occurrence of the second string in the first string with the third string and return the result |
|  | Not supported | REPLICATE | Return a string that repeats expr n times |
|  | Not supported | REVERSE_STR | Return the result of reversing the character order of expr |
|  | RPAD | RPAD | Append characters specified by argument expr2 to the right side of argument expr1 as many times as needed to reach n |
|  | RTRIM | RTRIM | Remove all characters specified by set from the right end of argument char |
...
|  | Not supported | STUFF | Remove length characters from the position specified by start and return the string with expr2 inserted at that position |
|  | SUBSTR | SUBSTR<br>SUBSTRING | Return a substring of expr with length characters starting at start |
|  | SUBSTRB | SUBSTRB | Determine position and length in bytes, not characters |
|  | TRANSLATE | TRANSLATE | Replace each character in from_string with the corresponding character in to_string and return expr |
|  | TREAT | Not supported | Change the declared type of the argument |
|  | TRIM | TRIM | Remove leading or trailing characters, or both, from a string |
|  | UPPER | UPPER | Convert all characters to uppercase |
...
|  | Not supported | DATE_TO_UNIX | Convert expr of DATE type to a value in seconds based on 1970-01-01 00:00:00 (UTC +00:00 time zone) and return it |
...
|  | REGEXP_COUNT | REGEXP_COUNT | Return the number of times the pattern occurs in a string |
...
| Datetime function | ADD_MONTHS | ADD_MONTHS | Return the value obtained by adding a specified number of months to date |
|  | Not supported | CONV_TIMEZONE | Convert expr from the src_tz time zone to the dest_tz time zone |
|  | CURRENT_DATE | CURRENT_DATE | Return the current session date information as a Date data type |
|  | CURRENT_TIMESTAMP | CURRENT_TIMESTAMP | Return the current session date and time information |
|  | +, - operation | +, -, DATEADD | Increase the date_field_name part of date by number and return the result |
|  | Not supported | DATEDIFF | Return enddate minus startdate in the unit specified by date_field_name |
|  | Not supported | DATENAME | Return the month or weekday name of date according to input date_field_name |
|  | DBTIMEZONE | DB_TIMEZONE | Return the database time zone value |
|  | EXTRACT (datetime) | DATEPART<br>EXTRACT (datetime) | Return only the value corresponding to date_field_name from the input date |
|  | FROM_TZ | Not supported | Convert a timestamp data type and time zone data type to a TIMESTAMP WITH TIME ZONE data type |
|  | LAST_DAY | LAST_DAY | Return the last day of the month that contains the date |
|  | LOCALTIMESTAMP | Not supported | Output the current date and time of timestamp |
|  | MONTHS_BETWEEN | MONTHS_BETWEEN | Calculate the months between date1 and date2 |
|  | NEW_TIME | Not supported | Output the zone1 time as zone2 time |
|  | NEXT_DAY | NEXT_DAY | Return the next date of the specified weekday based on the given day |
|  | NUMTODSINTERVAL | Not supported | Convert n to an INTERVAL DAY TO SECOND string |
|  | NUMTOYMINTERVAL | Not supported | Convert n to an INTERVAL YEAR TO MONTH string |
|  | ORA_DST_AFFECTED | Not supported |  |
|  | ORA_DST_CONVERT | Not supported |  |
|  | ORA_DST_ERROR | Not supported |  |
|  | ROUND (date) | ROUND (date) | Return the date rounded to the unit specified by format model fmt |
|  | SESSIONTIMEZONE | SESSION_TIMEZONE | Reflect the current session time zone |
|  | SYS_EXTRACT_UTC | Not supported | Return Coordinated Universal Time (UTC, formerly Greenwich Mean Time) |
|  | SYSDATE | SYSDATE | Return the date and time of the OS where the database resides |
|  | SYSTIMESTAMP | SYSTIMESTAMP | Return the system date |
|  | TRUNC (date) | TRUNC (date) | Round or truncate a date by year, month, or day |
|  | TZ_OFFSET | Not supported | Return the time zone offset corresponding to the argument based on the date when the statement is executed |
|  | Not supported | UNIX_DATE | Output the current date and time of the operating system based on UTC +00:00 |
|  | Not supported | UNIX_TIMESTAMP | Output the current date and time of the operating system based on UTC +00:00 |
|  | Not supported | UNIX_TO_DATE | Convert expr to DATE type and return it |
| Comparison function | CASE | CASE, CASE2 | If expr1 is true, return ret_expr1 |
...
| Conversion function | ASCIISTR | ASCIISTR | Return the ASCII string of a string |
...
|  | Not supported | BASE64_DECODE_STR | Decode the input string encoded in base64 and return the original data as a hexadecimal string |
|  | Not supported | BASE64_ENCODE | Encode a VARBYTE type value in base64 format and return it as a VARBYTE type string |
|  | Not supported | BASE64_ENCODE_STR | Return the result of base64-encoding a hexadecimal input string as a VARCHAR type string |
...
|  | Not supported | RAW_TO_VARCHAR | Convert a VARBYTE value converted from VARCHAR data by the TO_RAW function back to VARCHAR data |
|  | ROWIDTOCHAR | Not supported | Convert rowid value to VARCHAR2 format |
|  | ROWIDTONCHAR | Not supported | Convert rowid value to NVARCHAR2 format |
...
|  | TO_DSINTERVAL | Not supported | Convert to an INTERVAL DAY TO SECOND value |
|  | Not supported | TO_HEX | Convert n to hexadecimal |
|  | Not supported | TO_INTERVAL (NUMTODSINTERVAL) | Convert n to interval_unit units and return |
|  | TO_LOB | Not supported | Convert LONG or LONG RAW value to LOB value |
...
|  | TO_NCHAR (character) | TO_NCHAR (character) | Convert a string, CLOB, or NCLOB value to the national character set |
|  | TO_NCHAR (datetime) | TO_NCHAR (datetime) | Convert to the national character set |
|  | TO_NCHAR (number) | TO_NCHAR (number) | Convert n to the national character set |
...
|  | TRANSLATE ... USING | Not supported | Convert char to the specified character set for conversion between the database character set and the national character set |
|  | UNISTR | UNISTR | Take a text string as an argument and return it as the national character set |
...
|  | LNNVL | LNNVL | If the condition result is FALSE or NULL, return TRUE; if it is TRUE, return FALSE |
|  | NULLIF | NULLIF | If expr1 and expr2 are the same, return NULL |
|  | NVL | NVL | Replace NULL values with blanks in the query result |
|  | NVL2 | NVL2 | If expr1 is not NULL, NVL2 returns expr2; if expr1 is NULL, it returns expr3 |
...
|  | Not supported | SENDMSG | Send a message to ip-address and port as a Socket datagram |
|  | SYS_CONTEXT | SYS_CONTEXT | Return related parameter values by using the current session's environment information (context) as a namespace |
|  | SYS_GUID | SYS_GUID_STR | Create and return a globally unique identifier (RAW value) consisting of 16 bytes.<br>Create a globally unique 16-byte identifier and return it as a 32-character hexadecimal string |
...
|  | USER | USER_NAME | Return the session user name |
|  | USERENV | SESSION_ID | USERENV returns session information.<br>SESSION_ID returns the user's SESSION_ID |
| Aggregation functions | APPROX_COUNT_DISTINCT<br>APPROX_COUNT_DISTINCT_AGG<br>APPROX_COUNT_DISTINCT_DETAIL<br>APPROX_MEDIAN<br>APPROX_PERCENTILE<br>APPROX_PERCENTILE_AGG<br>APPROX_PERCENTILE_DETAIL | Not supported |  |
...
|  | STATS_BINOMIAL_TEST | Not supported | An exact probability test used for dichotomous variables where only two valid values exist (variables with two exclusive values) |
...
|  | STATS_T_TEST_ (STATS_T_TEST_ONE, STATS_T_TEST_PAIRED, STATS_T_TEST_INDEP and STATS_T_TEST_INDEPU) | Not supported | In a t-test, the significance of the difference between mean values is measured |
...
|  | Not supported | FIRST_VALUE_IGNORE_NULLS | Return the first row value excluding NULL values |
|  | LAG | LAG | Refer to a previous value relative to the current row |
|  | Not supported | LAG_IGNORE_NULLS | Return the first non-NULL value from the offset-th previous row relative to the current row |
|  | LAST_VALUE | LAST_VALUE | Return the last value in the ordered set within the window |
|  | Not supported | LAST_VALUE_IGNORE_NULLS | Return the last row value excluding NULL values |
|  | LEAD | LEAD | Refer to a following value relative to the current row |
|  | Not supported | LEAD_IGNORE_NULLS | Return the first non-NULL value from the offset-th following row relative to the current row |
|  | NTH_VALUE | NTH_VALUE | Return the value of the offset-th row |
|  | Not supported | NTH_VALUE_IGNORE_NULLS | Return the value of the offset-th row excluding NULL values |
|  | NTILE | NTILE | Divide output results into a user-specified number of groups |
|  | RATIO_TO_REPORT | RATIO_TO_REPORT | Calculate the ratio of a value to the sum of a set of values |
|  | ROW_NUMBER | ROW_NUMBER | Assign ranks to results sorted within each partition |
...
Altibase v5.5 vs ORACLE 10g comparison
