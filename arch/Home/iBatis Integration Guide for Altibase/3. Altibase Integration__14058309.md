...
As of ALTIBASE 5, Altibase.jar and Altibase5.jar files exist in the $ALTIBASE_HOME/lib directory. Altibase.jar is the general JDBC driver file, and Altibase5.jar is the JDBC driver file used when integrating Altibase v5 together with lower versions. Therefore, when integrating with one Altibase DB or multiple Altibase DBs of the same version, use the $ALTIBASE_HOME/lib/Altibase.jar file.
...
Project - JRE System Library (J2SE-1.5) - Properties - Installed JREs - click jre among the items - click Edit - Add External JARs to add Altibase.jar, Altibase JDBC Driver.
...
Ex) db.properties1 file of MultiVersionConnection
...
Settings for versions earlier than Altibase 5
...
Ex) MultiVersionConnection's SqlMapConfigExample1.xml file
...
Ex) MultiVersionConnection's SqlMapConfigExample2.xml file

Setting for versions earlier than ALTIBASE 5
...
In the PersonApp.java example above, to load Altibase5.jdbc.driver.AltibaseDriver before Altibase.jdbc.driver.AltibaseDriver, the application reads SqlMapConfigExample1.xml first because it uses Altibase5.jdbc.driver.AltibaseDriver as JDBC.Driver. The driver for Altibase 5 must be loaded first.
...
When calling a Stored Procedure/Function created in the DB from iBATIS, set the parameter information defined in the Stored Procedure/Function in the SqlMap file, and define the Procedure/Function call statement used by CallableStatement in the <procedure> tag.
