...
If TransactionManager is not specified or propagation is set to a value other than those described above when using declarative transaction, a null value is returned when querying LOB data, or an error such as "java.sql.SQLException: [0]:LobLocator can not span the transaction 101858625." occurs.

In addition, when entering LOB data, an error such as "java.sql.SQLException: [0]:LobLocator can not span the transaction 101858625." occurs.
...
To run the LobSpringIbatisSample project above, Altibase.jar, ibatis-2.3.4.x.jar, spring-jdbc.jar, spring-orm.jar, spring.jar, commons-logging.jar, and cglib-nodep-x.x.jar files are needed.
