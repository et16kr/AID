...
- Different .NET data provides depending on the ADO.NET version
...
Go to [http://support.altibase.com](http://support.altibase.com), go to "Download" -> "Product", download altiadonetX.X.X_32/64bit.zip from the LIBRARY Section, and extract the ADO.NET DLLs.

In order to get the same version as the server package, use the same version in the "CLIENT" section.

(If the previous version is not on the website, please contact Q&A bulletin board at [http://support.altibase.com/en)](http://support.altibase.com/en))
