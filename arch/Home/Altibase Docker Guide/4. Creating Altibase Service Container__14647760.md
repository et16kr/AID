...
Therefore, to ensure data persistence, use a data volume that mounts the host file system into the container.

The characteristics of the data volume are as follows.

- Data volume can be reused and shared between containers.
- Data volume can be accessed directly from the host.
- A data volume is essentially independent of the container, so it persists even if the container is deleted.
...
1. Create a Docker network. Create the Docker network with the `docker network create` command. Check the created network with the `docker network ls` command, and check details with the `docker inspect` command.
  ...
  Check the details of the created network.
  ...
  | -e SLAVE_HOST_NAME | Hostname of the additional node |
  | -e SLAVE_REP_PORT | Replication port number of the additional node |
