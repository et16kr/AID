...
The requirements and recommendations mentioned in this chapter assume a general-purpose database. They can vary depending on the user's database system and usage. For more information on system requirements, contact the Altibase customer service portal ([http://support.altibase.com](http://support.altibase.com/)).
...
When installing a patch, the Altibase Package Installer creates backups of the files to which the patch is applied and saves them in the $ALTIBASE_HOME/APatch directory. Separate backup directories are created for each patch in this directory. These backup files will be used later to roll back the patch.
...
| Linux | x86, x86-64 (GNU glibc 2.12 or later) | Red Hat 6.0 or later | 64-bit | 64 bit, 32-bit |
...
The Altibase property configuration stage is divided into the following three steps.
...
- Buffer size
- Specify whether to generate the SQL script to create a new database, which determines whether step 2 is performed. If YES is selected, step 2 is performed; otherwise, step 2 is skipped. Regardless of whether an SQL script is created by this process, a new database can be created with `server create [DB Character-set] [National Character-set]`.
...
To set the value of a property not included in this step, or to change a property value set in Step 1 or Step 3 later, manually edit the `$ALTIBASE_HOME/conf/altibase.properties` file.
...
- Post license registration: If a license key has not been obtained yet, obtain it later and copy the new license file to the $ALTIBASE_HOME/conf directory. However, if a license key is not registered during this process, Altibase will not ask whether to create a database in the next step.
  ```
...
The Altibase Package Installer provides the following two shell scripts and one SQL script to make it easier for users to set system kernel parameters and environment variables, create a database, and set up PSM.
