...
2. ### Check the SQL statement execution plan (PLAN)
  ...
  The prompt "iSQL" means that the command is executed in the SQL statement execution utility provided by Altibase. This utility is provided in `$ALTIBASE_HOME/bin/isql`. In the Windows environment, "Orange for Altibase" from WareValley and the open-source tool "DBeaver" can also be used. When the query is executed after the command above, the execution plan of the query is displayed.
...
1. Dynamic Method 4 is not supported.
2. Context syntax is not supported. Refer to "Considerations for Threads".
3. Procedure typeset, Ref-Cursor, and array-form result sets cannot be retrieved with the precompiler.
4. The client library is not safe for system signal handling. System signal processing must be handled by user code.
