...
This is the case that the connection object to be used does not exist or disappears during the query processing process. Check whether the connection has been properly processed and, in the case of a threaded program, check whether the connection name is properly indicated in the `EXEC SQL AT` clause. In some cases, the DBMS server may have disconnected the connection, so check timeout-related messages.
...
This is the case when conversion between the data type of the column used in Altibase and the host variable type is not supported.
...
This is the case when the date string entered in the date function and the date format do not match.
...
This is the case when the column is `NOT NULL` among the data values entered in the `INSERT` or `UPDATE` statement, and a NULL value is attempted to be inserted. Trace data values to prevent NULL data from occurring, or change the `NOT NULL` constraints of the column properly.
...
If it is set to a value of "0", the corresponding limit is not applied. However, as with `QUERY_TIMEOUT`, the transaction continues to occupy DBMS resources while it remains active and may cause a failure in some cases. Therefore, look for a way to resolve the root cause of the problem.
