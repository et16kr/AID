...
When compiled normally, a file called sample.cpp is created in the corresponding directory, and the prompt returns without any error. For detailed options related to `apre`, refer to "Altibase APRE(SES) *C/C++ Makefile".
...
The library options to add are `-lapre -lodbccli`. With this reference, a simple Makefile that compiles `connect1.sc` can be written as follows.
...
In addition to the ALTIBASE library, system libraries are required for compilation. For a list of system libraries required for each platform and Makefile examples, use the list defined in the `LIBS` tag in `$ALTIBASE_HOME/install/altibase_env.mk`.

One more point to consider is that if the user uses a C compiler instead of a C++ compiler, the ALTIBASE library is provided in C++, so the C++ system library must be linked for compatibility. For details, refer to "Altibase APRE(SES) *C/C++ Makefile".
...
```
  ...
  char   pwd[20];
  char   opt[200];
  // End of host variable declaration
  EXEC SQL END  DECLARE SECTION;
...
main()
{
    ...
    // Create connection for separate change transaction
...
| DO [user function] | Execute a user-defined function |
...
Starting from ALTIBASE version 5.3.3 or later, Session FailOver functions called CTF (Connection Time Failover) and STF (Service Time FailOver) are provided. CTF means failover when the program first attempts to connect, and STF means failover for a connection that occurs while executing a query after connection.
...
| Item | Description |
| --- | --- |
| AlternateServers | The target servers to FailOver are listed in `IP:PORT` form, in order, separated by commas (,). |
| ConnectionRetryCount | The number of connection attempts to one target server |
...
| LoadBalance | When turned on, the server specified in the DSN and the AlternateServers list are randomly selected at the time of initial connection. If turned off, the DSN is selected first, and then the connection is attempted in the order described in the AlternateServers list. |
