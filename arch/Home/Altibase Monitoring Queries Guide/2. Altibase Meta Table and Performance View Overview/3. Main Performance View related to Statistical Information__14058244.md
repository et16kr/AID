...
The `TIMED_STATISTICS` property has been provided since Altibase v5. Check whether this property is enabled with the following SQL statement.
...
- **v$lfg**
  ...
  The `LF_PREPARE_WAIT_COUNT` column indicates the number of times service threads waited because the next redo log file had not yet been created when switching from the current redo log file to a new redo log file. If this value is large, increase the `PREPARE_LOG_FILE_COUNT` property and apply it by restarting Altibase so that enough redo log files are created in advance.
